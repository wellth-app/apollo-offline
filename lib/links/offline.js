    ...
    request(operation, forward) {
        ...
        return new apollo_link_1.Observable((observer) => {
            ...
            if (isMutation) {
                ...
                if (!execute) {
                    const { [cache_1.METADATA_KEY]: { snapshot: { enqueuedMutations }, }, } = this.store.getState();
...
const processOfflineQuery = ({ query, variables, getContext }, store) => {
    const { [cache_1.NORMALIZED_CACHE_KEY]: normalizedCache = {} } = store.getState();
...
exports.offlineEffect = (store, client, effect, action, callback, mutationCacheUpdates) => __awaiter(this, void 0, void 0, function* () {
    ...
    const setQueryFunction = queryManager.setQuery;
    ...
    updateCacheWithEnqueuedMutationUpdates(enqueuedMutations, context, idsMap, mutationCacheUpdates, client);
    ...
    return new Promise((resolve, reject) => {
        ...
        observable.subscribe({
            next: (data) => {
                ...
                const { [cache_1.METADATA_KEY]: { idsMap, snapshot: { cache: cacheSnapshot }, }, } = store.getState();
...
const updateCacheWithEnqueuedMutationUpdates = (enqueuedMutations, context, idsMap, mutationCacheUpdates, client) => {
    enqueuedMutations
...
exports.offlineEffectConfig = {
    ...
    reducer: offlineEffect_1.default,
};
